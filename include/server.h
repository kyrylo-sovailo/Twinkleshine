...
struct Error *server_send_stream(struct ConstantValue *stream, int fd, enum ConnectionFlag *flags) NODISCARD;
/* Sends the message */
struct ExError server_send_message(struct ConstantValue *message, int fd, const struct sockaddr *address, unsigned int first_chunk, union Value *chunks, size_t last_chunk_size) NODISCARD;
/* Receives the given amount of bytes to ring */
struct ExError server_receive_stream(struct Ring *stream, int fd, size_t size, enum ConnectionFlag *flags) NODISCARD;
...
struct ExError server_receive_traffic(struct Client *client, int fd, time_t now, enum ConnectionFlag *flags) NODISCARD;

/* process.c */
/* Parses the data in the input buffer, and if the request is complete, processes it and pushes into output buffer */
struct ExError server_process_traffic(struct Client *client, time_t now) NODISCARD;

/* send.c */
/* Sends the client's output buffer */
struct ExError server_send_traffic(struct Client *client, int fd, time_t now, enum ConnectionFlag *flags) NODISCARD;
