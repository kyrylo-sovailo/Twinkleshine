...
static struct Error *processor_process_generic(unsigned char accepting_socket, struct ProcessorPrintContext *context, const struct Value *resource, bool *invalid_resource)
{
    ...
    else if (value_compare_case_mem(&trimmed_resource, STRING_STRLEN("twinkleshine")))
    {
        ...
        PRET(processor_print(a, context, ES_NORMAL, NULL, "Another potential application of Twinkleshine would be to delegate it network handling and to run some custom modules. And do so efficiently, since no inter-process communication is taking place."));
    ...
    else if (value_compare_case_mem(&trimmed_resource, STRING_STRLEN("smallnet")))
    {
        ...
        PRET(processor_print(a, context, ES_NORMAL, NULL, "Other protocols (Gopher, Finger, Gemini) can be used to access information too. They are collectively known as 'smallnet'. And this is how smallnet looks like - it's mostly text."));
        PRET(processor_print(a, context, ES_LARGE,  NULL, "But why?"));
        PRET(processor_print(a, context, ES_NORMAL, NULL, "Why would anyone want to reject pictures?"));
        PRET(processor_print(a, context, ES_NORMAL, NULL, "For the same reason you don't see pictures in the books and don't hear music in the library. It is distracting."));
        ...
        PRET(processor_print(a, context, ES_LARGE,  NULL, "How can I join?"));
        PRET(processor_print(a, context, ES_NORMAL, NULL, "Most popular browsers don't support smallnet protocols. Firefox once supported Gopher, not anymore. Smallnet pages can be opened in special browsers, including:"));
        ...
        PRET(processor_print(a, context, ES_NORMAL, NULL, "Be aware though that none (to my knowledge) smallnet browsers support HTTP(S). Here are some alterative options:"));
