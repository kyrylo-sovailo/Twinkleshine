...
struct Error *processor_print_gemini(struct ProcessorPrintContext *context, enum EntryStyle style, const char *resource, const char *format, va_list va)
{
    ...
    case ES_INTERNAL_REFERENCE: PRET(string_print_append(context->one, "=> gemini://" DOMAIN_NAME GEMINI_PORT_STRING "/%s ", resource)); break;
    ...
    struct Response *response, struct Ring *response_queue, struct Value *response_stream)
{
    ...
    *response_stream = zero;
    ...
    response_stream->parts[0].size = sizeof("40 Invalid resource" CRLF) - 1;
...
void processor_fixed_gemini_failsafe(enum FixedResponse fixed, struct Value *response_stream)
{
    const struct Value zero = ZERO_INIT;
