    ...
    struct Response *response, struct Ring *response_queue, struct ConstantValue *response_stream)
{
    ...
    if (!value_compare_case_mem(&method.c, STRING_STRLEN("get")))
    {
    ...
    else if (value_compare_case_mem(&resource.c, STRING_STRLEN("/robots.txt")))
    {
    ...
    *response_stream = zero;
    ...
    EXPRET(processor_construct_response(response, response_queue,
        value_size(response_stream), keep_alive, 0, 0, &method.c, &resource.c, &protocol.c));
    ...
    struct ConstantContinuousValue *response_stream)
{
    ...
    response_stream->p = fixed_string->p;
    response_stream->size = fixed_string->size;
}
...
{
    /* Pre-prefix*/
    struct tm global_calender;
    if (context->previous_style == ES_ITEMIZE && style != ES_ITEMIZE) { PRET(string_append_mem(context->two, STRING_STRLEN("</ul>" ENDLINE))); }
    if (context->previous_style == ES_ENUMERATION && style != ES_ENUMERATION) { PRET(string_append_mem(context->two, STRING_STRLEN("</ol>" ENDLINE))); }
    ...
    case ES_INITIALIZE:
        ...
        return OK;
    case ES_FINALIZE:
        global_calender = time_to_calender(time(NULL), true);
    ...
    case ES_ITEMIZE:
        ...
        break;
    case ES_ENUMERATION:
        if (context->previous_style != ES_ENUMERATION) { PRET(string_append_mem(context->two, STRING_STRLEN("<ol>" ENDLINE))); }
        PRET(string_append_mem(context->two, STRING_STRLEN("<li>")));
        break;
    ...
    switch (style)
    {
    case ES_NORMAL: PRET(string_append_mem(context->two, STRING_STRLEN("<br>" ENDLINE))); break;
    case ES_PARAGRAPH: PRET(string_append_mem(context->two, STRING_STRLEN("</p>" ENDLINE))); break;
    case ES_ITEMIZE:
