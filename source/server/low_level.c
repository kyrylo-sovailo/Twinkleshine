...
struct Error *server_send_value(struct ConstValue *value, int fd, bool *connection_saturated)
{
    ...
    for (i = 0; i < VALUE_PARTS; i++)
    {
        ...
        if (signed_sent > 0)
        {
            ...
            if (value->parts[i].size > 0) { *connection_saturated = true; break; }
        }
        else
        {
            ARET0(errno == EAGAIN || errno == EWOULDBLOCK, "send() failed");
            *connection_saturated = true;
            break;
        }
    }
    return OK;
...
struct ExError server_receive_value(struct Ring *ring, int fd, size_t size, bool *connection_exhausted, bool *connection_closed)
{
